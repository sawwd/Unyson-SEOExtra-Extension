...
class FW_Option_Type_SEO_Tags extends FW_Option_Type {
	...
	protected function _enqueue_static( $id, $option, $data ) {
		$css_uri    = fw()->extensions->get( 'seo-extra' )->get_declared_URI( '/includes/option-types/' . $this->get_type() . '/static/css/style.css' );
		$js_uri     = fw()->extensions->get( 'seo-extra' )->get_declared_URI( '/includes/option-types/' . $this->get_type() . '/static/js/scripts.js' );
		$seo_tags   = fw()->extensions->get( 'seo-extra' )->get_seo_tags();
		$tags_names = array();
		...
		wp_enqueue_script( 'fw-option-' . $this->get_type(), $js_uri, array( 'jquery', 'jquery-ui-autocomplete' ),
			$version, true );
	...
	protected function _get_value_from_input( $option, $input_value ) {
		return fw()->backend->option_type( 'text' )->get_value_from_input( $option, $input_value );
	}
}
